...
if page == "1. Data Input":
    ...
    if all_files_uploaded:
        ...
        if st.session_state.data_processed: # Only reset if it was previously processed
            st.session_state.data_processed = False
            st.session_state.weather_df = pd.DataFrame()
            st.session_state.uploaded_files = current_file_names
...
# --- Page 2: Data Visualization ---
    ...
    if st.session_state.weather_df.empty:
        ...
        if visualization_type == "Total Distribution":
            ...
            params_list = list(parameters_to_plot.items())
            
            for i in range(0, len(params_list), 2):
                cols = st.columns(2 if i + 1 < len(params_list) else 1) # Create 2 columns, or 1 for the last one if odd
                
                # Plot first parameter in the row
                param1, categories1 = params_list[i]
                with cols[0]:
                    st.subheader(f"Distribution of {param1.replace('Categorized_', '')} ({start_year} - {end_year})")
                    if not filtered_df.empty:
                        category_counts1 = filtered_df[param1].value_counts().reindex(categories1)
                        plot_df1 = category_counts1.reset_index()
                        plot_df1.columns = [param1, 'Count']
                        fig1, ax1 = plt.subplots(figsize=(5, 3)) # Slightly larger for better readability
                        sns.barplot(data=plot_df1, x=param1, y='Count', palette='viridis', ax=ax1, order=categories1)
                        ax1.set_title(f'Total Count by Category for {param1.replace("Categorized_", "")}')
                        ax1.set_xlabel(param1.replace("Categorized_", ""))
                        ax1.set_ylabel("Total Count of Months")
                        plt.xticks(rotation=10, ha='right')
                        plt.tight_layout()
                        st.pyplot(fig1)
                    else:
                        st.warning(f"No data available for {param1.replace('Categorized_', '')} in the selected year range.")
                
                # Plot second parameter in the row, if it exists
                if i + 1 < len(params_list):
                    param2, categories2 = params_list[i+1]
                    with cols[1]:
                        st.subheader(f"Distribution of {param2.replace('Categorized_', '')} ({start_year} - {end_year})")
                        if not filtered_df.empty:
                            category_counts2 = filtered_df[param2].value_counts().reindex(categories2)
                            plot_df2 = category_counts2.reset_index()
                            plot_df2.columns = [param2, 'Count']
                            fig2, ax2 = plt.subplots(figsize=(5, 3)) # Slightly larger
                            sns.barplot(data=plot_df2, x=param2, y='Count', palette='viridis', ax=ax2, order=categories2)
                            ax2.set_title(f'Total Count by Category for {param2.replace("Categorized_", "")}')
                            ax2.set_xlabel(param2.replace("Categorized_", ""))
                            ax2.set_ylabel("Total Count of Months")
                            plt.xticks(rotation=10, ha='right')
                            plt.tight_layout()
                            st.pyplot(fig2)
                        else:
                            st.warning(f"No data available for {param2.replace('Categorized_', '')} in the selected year range.")
            ...
            filtered_df_trend = st.session_state.weather_df[(st.session_state.weather_df['Year'] >= start_year) & (st.session_state.weather_df['Year'] <= end_year)]
            ...
            for i in range(0, len(params_list), 2):
                ...
                param1, categories1 = params_list[i]
                with cols[0]:
                    st.subheader(f"Yearly Trend of {param1.replace('Categorized_', '')}")
                    if not filtered_df_trend.empty:
                        plot_df1 = filtered_df_trend.groupby(['Year', param1]).size().reset_index(name='Count')
                        fig1, ax1 = plt.subplots(figsize=(6, 4)) # Adjusted size for yearly trend
                        sns.barplot(data=plot_df1, x='Year', y='Count', hue=param1, palette='viridis', ax=ax1, order=range(start_year, end_year + 1))
                        ax1.set_title(f'Category Distribution per Year for {param1.replace("Categorized_", "")}')
                        ax1.set_xlabel("Year")
                        ax1.set_ylabel("Count of Months")
                        plt.xticks(rotation=45, ha='right', fontsize=8)
                        plt.legend(title=param1.replace("Categorized_", ""), bbox_to_anchor=(1.05, 1), loc='upper left')
                        plt.tight_layout()
                        st.pyplot(fig1)
                    else:
                        st.warning(f"No data available for {param1.replace('Categorized_', '')} in the selected year range.")
                ...
                if i + 1 < len(params_list):
                    param2, categories2 = params_list[i+1]
                    with cols[1]:
                        st.subheader(f"Yearly Trend of {param2.replace('Categorized_', '')}")
                        if not filtered_df_trend.empty:
                            plot_df2 = filtered_df_trend.groupby(['Year', param2]).size().reset_index(name='Count')
                            fig2, ax2 = plt.subplots(figsize=(6, 4)) # Adjusted size for yearly trend
                            sns.barplot(data=plot_df2, x='Year', y='Count', hue=param2, palette='viridis', ax=ax2, order=range(start_year, end_year + 1))
                            ax2.set_title(f'Category Distribution per Year for {param2.replace("Categorized_", "")}')
                            ax2.set_xlabel("Year")
                            ax2.set_ylabel("Count of Months")
                            plt.xticks(rotation=45, ha='right', fontsize=8)
                            plt.legend(title=param2.replace("Categorized_", ""), bbox_to_anchor=(1.05, 1), loc='upper left')
                            plt.tight_layout()
                            st.pyplot(fig2)
                        else:
                            st.warning(f"No data available for {param2.replace('Categorized_', '')} in the selected year range.")
...
# --- Page 3: Predict Weather ---
    ...
    if st.session_state.weather_df.empty:
        ...
        year_input = st.number_input("Enter a year for prediction:", min_value=1961, max_value=2023, value=2023, step=1)
        ...
        if st.button("Predict"):
            if 'models' in st.session_state:
                ...
                units = {
                    ...
                    'MinTemp': '°C',
                    'Rainfall': 'mm',
                    'CloudCoverage': 'oktas', # Oktas is a unit for cloud coverage (0-8)
                    'Humidity': '%',
                    'Sunshine': 'hours',
                ...
                for param, model in st.session_state.models.items():
                    ...
                    unit = units.get(param, '') # Get unit, default to empty string if not found
                    st.write(f"**{param}:** {prediction:.2f} {unit}")
...
# --- Page 4: Report Generation ---
    ...
    if 'prediction_results' not in st.session_state:
        ...
        # Display the prediction results again (optional, for on-screen review)
        ...
        units = { # Define units again for display in Streamlit on this page
            'MaxTemp': '°C',
            'MinTemp': '°C',
            'Rainfall': 'mm',
            'CloudCoverage': 'oktas',
        ...
        for param, value in st.session_state.prediction_results.items():
            ...
            st.write(f"{param}: {value:.2f} {unit}")
        ...
        def create_pdf(prediction_data):
            ...
            # Define custom styles for report
            styles.add(ParagraphStyle(name='CenteredTitle', alignment=TA_CENTER, fontSize=24, spaceAfter=20))
            styles.add(ParagraphStyle(name='Heading', alignment=TA_CENTER, fontSize=18, spaceAfter=16))
            styles.add(ParagraphStyle(name='InputParam', fontSize=14, spaceAfter=6, fontName='Helvetica-Bold'))
            styles.add(ParagraphStyle(name='PredictionParam', fontSize=14, spaceAfter=6, fontName='Helvetica')) # Removed bold for values, kept for param name
            ...
            # Define units for PDF generation
            pdf_units = {
                'MaxTemp': '°C',
                'MinTemp': '°C',
                'Rainfall': 'mm',
            ...
            for param, value in prediction_data['results'].items():
                unit = pdf_units.get(param, '')
                # Use HTML-like tags for bolding parameter name within Paragraph
                story.append(Paragraph(f"<b>{param}:</b> {value:.2f} {unit}", styles['PredictionParam']))
